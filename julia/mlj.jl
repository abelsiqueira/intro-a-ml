...
# ╔═╡ 5628c458-bad1-4464-a14d-81ff238f8c18
using CSV, DataFrames, Plots, MLJ, MLJLinearModels, MLJDecisionTreeInterface, LinearAlgebra, Random, MLJNaiveBayesInterface, MLJLIBSVMInterface, NearestNeighborModels, LIBSVM

# ╔═╡ 8ee68ceb-3fa2-4002-a060-c9b51a295e27
begin
	pessoas_file = download("https://raw.githubusercontent.com/abelsiqueira/intro-a-ml/main/data/pessoas.csv", "pessoas.csv")
	df = CSV.read(pessoas_file, DataFrame)
end

# ╔═╡ 17e95eca-8b4e-4d7c-90d2-2627903ed3c2
begin
	y, X = unpack(
		df,
		==(:salario),
	...
	if size(X, 2) == 1
		X = coerce(df[!,[:idade]], :idade => Continuous)
	end
	X[!,:idade_pow2] = X.idade .^ 2

	hot = OneHotEncoder()
	hot_mach = machine(hot, X)
	MLJ.fit!(hot_mach)
	X = MLJ.transform(hot_mach, X)

	schema(X)
end

# ╔═╡ 28665e2c-4abb-41fd-98c3-f0cabb402255
train, test = partition(1:length(y), 0.7, rng=123)

# ╔═╡ 5f26cffd-9001-4976-a689-b2ae164e9d62
# ╠═╡ show_logs = false
let
	LinearRegressor = @load LinearRegressor pkg=MLJLinearModels verbosity=0
	model = LinearRegressor()
	cv = CV(nfolds = 5)
	evaluate(model, X, y, resampling = cv, measure = rsq)
	# mach = machine(model, X, y)
	# MLJ.fit!(mach, rows=train)
	# ŷ = MLJ.predict(mach, X[test,:])

	# compara2d(X[test,:], y[test], ŷ, "LinearRegressor")
end

# ╔═╡ 5cbf9be2-01a8-4d96-9a4b-c1a033b4695f
MLJ.models(matching(X, y))

# ╔═╡ c3425abf-8c47-4674-b550-4123947573b2
begin
	function compara2d!(plt, X, y, ŷ, nome = "no name")
		R2 = round(rsq(ŷ, y), digits=4)
		scatter!(plt, X.idade, y, lab="real")
		scatter!(plt, X.idade, ŷ, lab="pred")
		title!(plt, "$nome, R2 = $R2")
	end

	function compara2d(args...; kwargs...)
		plt = plot()
		compara2d!(plt, args...; kwargs...)
		return plt
	end
end

# ╔═╡ 298cc300-082c-490b-b71b-4714e1b7d9ce
let
	LinearRegressor = @load LinearRegressor pkg=MLJLinearModels verbosity=0
	...
	for (i, reg) in enumerate([
		...
		compara2d!(plt[i], X[test,:], y[test], ŷ, "$reg")
	end
	plt
end

# ╔═╡ 5dc2c937-2a7d-4319-8a61-610241c8b1de
let
	...
	compara2d(X[test,:], y[test], ŷ, "RandomForestRegressor")
end

# ╔═╡ a0096a1c-dc60-422c-9d67-4bd1d431cb75
begin
	Random.seed!(0)
	...
	for j = 1:5
		...
		Σ = rand(2, 2)
		Σ = Σ' * Σ + 0.3I
		X2 = [X2; randn(100, 2) * Σ .+ μ']
	end
	X2 = DataFrame(X2, :auto)
	n = size(X2, 1)
	y2 = coerce([
		-(X2[i,2] - 0.5) * X2[i,1] > randn() * 1.5
		for i = 1:n
	], OrderedFactor)
	X2 = X2 .* [1000 3] .+ [10_000 10]

	function plot_2d_class!(plt, X, y)
		yu = unique(y)
	    colors = [:red, :blue, :green]
	    if length(yu) > length(colors)
	        error("Abel, mais cores!")
	    end
	    for (i, yi) in enumerate(yu)
	        idx = findall(y .== yi)
	        scatter!(plt, X[idx,1], X[idx,2], c=colors[i], lab="$yi", opacity=0.7, m=(4,stroke(0)))
	    end
	    plt
	end
	
	function plot_machine_class!(plt, mach, X; use_pdf = false)
	    N = 51
	    X1 = range(extrema(X[:,1])..., length=N)
	    X2 = range(extrema(X[:,2])..., length=N)
	    Xg = DataFrame(vcat([
	        [X1[i]; X2[j]]'
	        for i = 1:N, j = 1:N
	    ][:]...), names(X))
	    ŷ = MLJ.predict(mach, Xg)
	    if use_pdf
	        ŷ = pdf.(ŷ, true)
	    end
	    contourf!(plt, X1, X2, ŷ, c=cgrad([:pink, :white, :lightblue]), lw=0)
	    contour!(plt, X1, X2, ŷ, lw=2, c=:black, levels=[0.5])
	end
	
	plot_2d_class!(plot(), X2, y2)
end

# ╔═╡ 3e6470ec-aa64-49b5-8996-dd3fe407a6e7
begin
...
# ╔═╡ cad53c61-fe06-4052-a127-e6534f660431
let
	using Logging
	...
	use_pdfs = [true, true, false, true, true, true]
	
	cv = CV(nfolds = 3)
	nclfs = length(clfs)
	plts = plot(
	    layout = grid(ceil(Int, 0.5nclfs), 2),
	    size = (1200, 400 * ceil(Int, 0.5nclfs)),
	    leg = false
	)
	...
	for (i, clf) in enumerate(clfs)
	    ...
	    plot_machine_class!(plts[i], mach, X2, use_pdf=use_pdfs[i])
	    plot_2d_class!(plts[i], X2, y2)
	    acc = round(ev.measurement[1], digits = 2)
	    f1s = round(ev.measurement[2], digits = 2)
	    name = string(clf)
	    name = name[1:min(5,length(name))]
	    title!(plts[i], "$(name)..., acc = $acc, f1s = $f1s")
	end
	plts
end

# ╔═╡ 24a39a09-d31c-4511-93e0-dee43dcd0bb3
let
	# model = SVC(
	...
	plt = plot()
	plot_machine_class!(plt, mach, X2; use_pdf = false)
	plot_2d_class!(plt, X2, y2)
end
...
manifest_format = "2.0"
project_hash = "1f6c6682630b397d553d5ec17625a6633ebe600b"
...
git-tree-sha1 = "19609e3e1479f419e4077ffba91f9af480c5c015"
...
git-tree-sha1 = "d5b544e3632f08346219d47f2abcfa25306a5f7e"
uuid = "a7f614a8-145f-11e9-1d2a-a57a1082229d"
version = "0.20.7"

[[deps.MLJDecisionTreeInterface]]
deps = ["DecisionTree", "MLJModelInterface", "Random", "Tables"]
git-tree-sha1 = "fb5a1e6f7314c68b785d7ea3b1fbfa2751386295"
...
git-tree-sha1 = "35446972ff751e2f9fd23ca140107a69e325882f"
...
git-tree-sha1 = "6843e87558e6cffbfa21b8b3a438541f0ffbabbf"
...
git-tree-sha1 = "218e38c3f3bddc64e5f365476923c65f1ae19d68"
